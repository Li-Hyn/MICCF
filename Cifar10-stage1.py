...
from model.resnet_cifar import ResNet18withSobel
...
from losses.Loss_IMI import crossview_contrastive_Loss_cifar
...
def main():
    ...
    npc, loss_id, loss_fmi = npc.to(device), loss_id_func.to(device), loss_fmi_func.to(device)
    # lr = 0.03
    ...
    file_name = "Cifar10-stage1_test1.py"
    ...
    for epoch in range(start_epoch+1, epochs + 1):
            ...
            for pos_1, pos_2, target, index in train_bar:
                ...
                loss_imi = crossview_contrastive_Loss_cifar(features_1, features_2)

                loss_id = loss_id_func(outputs, indexes)
                loss_fmi = loss_fmi_func(features_1)
                #loss_fmi = loss_fmi(features_2)

                tot_loss = loss_id + 0.01 * loss_fmi + 0.0001 * loss_imi
                tot_loss.backward()
            ...
            if (epoch == 0) or (((epoch + 1) % 10) == 0):
                ...
                print("Epoch:{} Loss_id:{} loss_fmi:{} loss_imi:{} Kmeans ACC, NMI, ARI = {}, {}, {}".format(epoch+1, loss_id, loss_fmi, loss_imi, acc, nmi, ari))
                ...
                torch.save({'optimizer': optimizer.state_dict(), 'model': net.state_dict(),
                            'epoch': epoch + 1, 'npc': npc.state_dict()}, args.pretrained)
